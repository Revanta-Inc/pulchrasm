...
char AA_NAMES[21][4] = {"GLY", "ALA", "SER", "CYS", "VAL", "THR", "ILE",
...
real rnd(void) { return 0.001 * (real)(rand() % 1000); }
...
void delete_mol(mol_type *mol) {
  ...
  if (mol->prev)
    mol->prev->next = mol->next;
  if (mol->next)
    mol->next->prev = mol->prev;
  if (mol->name)
    free(mol->name);
  if (mol->residua) {
    while (mol->residua) {
...
char setseq(char *aaname) {
  ...
  for (i = 0; i < 21; i++) {
    if ((aaname[0] == AA_NAMES[i][0]) && (aaname[1] == AA_NAMES[i][1]) &&
...
int res_contact(res_type *res1, res_type *res2) {
  ...
  while (atoms1) {
    ...
    while (atoms2) {
      ...
      if ((atoms1->flag & FLAG_SIDECHAIN) && (atoms2->flag & FLAG_SIDECHAIN) &&
...
int read_pdb_file(char *filename, mol_type *molecules, char *realname) {
  ...
  res = NULL;
  while (!feof(inp)) {
    if (fgets(buffer, 1000, inp) != buffer)
      break;
    if (!strncmp(buffer, "END", 3) || !strncmp(buffer, "TER", 3))
    ...
    if (!strncmp(buffer, "ATOM", 4) || !strncmp(buffer, "HETATM", 6)) {
      ...
      if (resnum == prevresnum && !strncmp(atmname, "N ", 2)) {
        if (_VERBOSE)
          printf("WARNING: fault in numeration at residuum %s[%d]\n", resname,
                 resnum);
        warn = 1;
      }
      if (atmname[0] == 'H')
...
real calc_ca_energy(atom_type **c_alpha, real **new_c_alpha,
                    real **init_c_alpha, real **gradient, real alpha, real *ene,
                    bool calc_gradient) {
  int i, j;
  real dx, dy, dz;
...
void ca_optimize(char *tname, char *iname) {
  ...
  int *sec;
  real **new_c_alpha, **gradient, **init_c_alpha, last_alpha, tmp,
  ...
  if (_VERBOSE)
    printf("Alpha carbons optimization...\n");
    ...
    while (atom) {
      if (atom->name[0] == 'C' && atom->name[1] == 'A') {
        if (i < chain_length) {
          ...
          if (_VERBOSE)
            printf(
                "WARNING: number of C-alpha atoms exceeds the chain length!\n");
          break;
        }
      }
      atom = atom->next;
  ...
  if (_CISPRO) {
    for (i = 1; i < chain_length; i++) {
      ...
      if ((setseq(c_alpha[i]->res->name) == 'P') &&
  ...
  if (_CA_RANDOM) {
    if (_VERBOSE)
      printf("Generating random C-alpha coordinates...\n");
    c_alpha[0]->x = 0.0;
    c_alpha[0]->y = 0.0;
    c_alpha[0]->z = 0.0;
    for (i = 1; i < chain_length; i++) {
  ...
  if (iname) {
    ...
    if (inp) {
      if (_VERBOSE)
        printf("Reading initial structure %s...\n", iname);
      i = 0;
      while (!feof(inp)) {
        if (fgets(buf, 1000, inp) == buf && buf[13] == 'C' && buf[14] == 'A') {
          if (i < chain_length) {
            ...
            if (_VERBOSE)
              printf("WARNING: number of ini-file C-alpha atoms exceeds the "
                     "chain length!\n");
            break;
          }
        }
      ...
      printf("WARNING: can't read initial corrdinates %s\n", iname);
  }
  ...
  do {
      ...
      if (out) {
        ...
        for (i = 0; i < chain_length; i++) {
          fprintf(out, "ATOM  %5d  %-3s %3s %c%4d    %8.3f%8.3f%8.3f\n", i + 1,
                  "CA ", c_alpha[i]->res->name, ' ', c_alpha[i]->res->num,
    ...
    e_pot = calc_ca_energy(c_alpha, new_c_alpha, init_c_alpha, gradient, 0.0,
                           energies, true);

    if (_VERBOSE && !init) {
      printf("Initial energy: bond=%.5lf angle=%.5f restraints=%.5f xvol=%.5f "
             "total=%.5f\n",
    ...
    ene1 = calc_ca_energy(c_alpha, new_c_alpha, init_c_alpha, gradient, alpha1,
                          energies, false);
    ene2 = calc_ca_energy(c_alpha, new_c_alpha, init_c_alpha, gradient, alpha2,
                          energies, false);
    ene3 = calc_ca_energy(c_alpha, new_c_alpha, init_c_alpha, gradient, alpha3,
                          energies, false);
    ...
    while (ene2 > MIN(ene1, ene3) && msteps < _CA_ITER) {
      ...
      alpha1 *= 2.0;
      alpha3 *= 2.0;
      ene1 = calc_ca_energy(c_alpha, new_c_alpha, init_c_alpha, gradient,
                            alpha1, energies, false);
      ene3 = calc_ca_energy(c_alpha, new_c_alpha, init_c_alpha, gradient,
                            alpha3, energies, false);
    }
    ...
    do {
      if (alpha3 - alpha2 > alpha2 - alpha1) {
        ...
        e0 = calc_ca_energy(c_alpha, new_c_alpha, init_c_alpha, gradient, a0,
                            energies, false);
        e0 = calc_ca_energy(c_alpha, new_c_alpha, init_c_alpha, gradient,
                            a0 - 1e-5, energies, false);
        e0 = calc_ca_energy(c_alpha, new_c_alpha, init_c_alpha, gradient,
                            a0 + 1e-5, energies, false);
        e0 = calc_ca_energy(c_alpha, new_c_alpha, init_c_alpha, gradient, a0,
                            energies, false);
    ...
    for (i = 0; i < chain_length; i++) {
      c_alpha[i]->x =
          c_alpha[i]->x +
          (last_alpha + last_alpha * (rnd() - 0.5) * eps) * gradient[i][0];
      c_alpha[i]->y =
          c_alpha[i]->y +
          (last_alpha + last_alpha * (rnd() - 0.5) * eps) * gradient[i][1];
      c_alpha[i]->z =
          c_alpha[i]->z +
          (last_alpha + last_alpha * (rnd() - 0.5) * eps) * gradient[i][2];
    }
  ...
  if (_VERBOSE)
    printf("Optimization done after %d step(s).\nFinal energy: bond=%.5lf "
...
void write_pdb(char *name, mol_type *mol) {
  ...
  while (res) {
      ...
      if (!_BB_REARRANGE) {
        ...
        while (atom) {
          if (!(atom->name[0] == 'D' && atom->name[1] == 'U') &&
            ...
            fprintf(out, "ATOM  %5d  %-3s %3s %c%4d    %8.3f%8.3f%8.3f\n",
                    anum++, atom->name, res->name, ' ', res->num, atom->x,
                    atom->y, atom->z);
    ...
    if (!res->next && oxt) {
      atom = oxt;
      fprintf(out, "ATOM  %5d  %-3s %3s %c%4d    %8.3f%8.3f%8.3f\n", anum++,
              atom->name, res->name, ' ', res->num, atom->x, atom->y, atom->z);
    }
    res = res->next;
  }
  fprintf(out, "TER\nEND\n");
  ...
  while (res) {
    if (res->protein) {
      ...
      while (atom) {
        if ((atom->name[0] == 'C' && atom->name[1] == 'A'))
          fprintf(out, "ATOM  %5d  %-3s %3s %c%4d    %8.3f%8.3f%8.3f\n", anum++,
                  atom->name, res->name, ' ', res->num, atom->x, atom->y,
                  atom->z);
        atom = atom->next;
      }
      fprintf(out, "ATOM  %5d  %-3s %3s %c%4d    %8.3f%8.3f%8.3f\n", anum++,
              "CM ", res->name, ' ', res->num, res->cmx, res->cmy, res->cmz);
    }
    res = res->next;
  }
  fprintf(out, "TER\nEND\n");
...
// r14 chiral distance
real calc_r14(real x1, real y1, real z1, real x2, real y2, real z2, real x3,
              real y3, real z3, real x4, real y4, real z4) {
  real r, dx, dy, dz;
...
real superimpose2(real **coords1, real **coords2, int npoints, real **tpoints,
                  int ntpoints) {
...
void rebuild_backbone(void) {
  ...
  for (i = 0; i < chain_length + 1; i++) {
    ...
    if (prevres) {
      add_replace(prevres, "C  ", tmpcoords[4][0], tmpcoords[4][1],
                  tmpcoords[4][2], FLAG_BACKBONE);
      add_replace(prevres, "O  ", tmpcoords[5][0], tmpcoords[5][1],
                  tmpcoords[5][2], FLAG_BACKBONE);
    }

    if (res) {
      add_replace(res, "N  ", tmpcoords[6][0], tmpcoords[6][1], tmpcoords[6][2],
                  FLAG_BACKBONE);
    } else { // terminal oxygen instead of nitrogen
      add_replace(prevres, "OXT", tmpcoords[6][0], tmpcoords[6][1],
                  tmpcoords[6][2], FLAG_BACKBONE);
    }
...
int read_rotamers(void) {
  ...
  if (_VERBOSE)
    printf("Reading rotamer library...\n");
  ...
  while (!feof(inp)) {
    if (fgets(buf, 1000, inp) == buf) {
      ...
      for (i = 0; i < l; i++) {
        new_rot->coords[i] =
            (real **)calloc(sizeof(real *) * (nheavy[aa] + 1), 1);
...
void rebuild_sidechains(void) {
  ...
  for (i = 0; i < chain_length; i++) {
    ...
    for (j = 1; j < nsc; j++) {
      add_replace(res, heavy_atoms[10 * res->type + j - 1], sc[j][0], sc[j][1],
                  sc[j][2], FLAG_SIDECHAIN);
    }
    ...
    for (ii = i - 2; ii <= i + 2; ii++)
      for (jj = j - 2; jj <= j + 2; jj++)
        for (kk = k - 2; kk <= k + 2; kk++) {
          if (ii >= 0 && ii < xgrid && jj >= 0 && jj < ygrid && kk >= 0 &&
              kk < zgrid) {
            llist = grid[ii][jj][kk];
            while (llist) {
              atom2 = llist->atom;
                ...
                if (!merged) {
                  ...
                  if (dd < 1.6 * 1.6) {
                    printf("STERIC CONFLICT: %s[%d]%s-%s[%d]%s\n",
                           atom->res->name, atom->res->num, atom->name,
                           atom2->res->name, atom2->res->num, atom2->name);
                    con++;
                  }
                }
              }
        ...
        if (con > 0) {
          ...
          for (l = 0; l < 10; l++) { // check two closest conformations
            ...
            for (m = 0; m < rot_stat_idx[cpos][4]; m++) {
              ...
              for (j = 1; j < nsc; j++) {
                add_replace(res, heavy_atoms[10 * res->type + j - 1], sc[j][0],
                            sc[j][1], sc[j][2], FLAG_SIDECHAIN);
              }
              con = get_conflicts(res, grid, xgrid, ygrid, zgrid);
              // printf("test: %d\n", con);
...
void vcross(real ax, real ay, real az, real bx, real by, real bz, real *cx,
            real *cy, real *cz) {
  *cx = ay * bz - by * az;
  *cy = az * bx - bz * ax;
  *cz = ax * by - bx * ay;
...
int chirality_check(void) {
  ...
  while (res) {
    ...
    if (a_ca && a_n && a_c && a_cb) {
      ...
      if (angle < 0.) {
        if (_VERBOSE)
          printf("WARNING: D-aa detected at %s %3d : %5.2f", res->name,
                 res->num, angle);
        xx = a_ca->x - a_n->x;
        xy = a_ca->y - a_n->y;
        xz = a_ca->z - a_n->z;
...
real hb_energy(res_type *res, atom_list ****grid, int xgrid, int ygrid,
               int zgrid) {
  atom_type *atom, *c_atom1, *o_atom1, *n_atom1, *c_atom2, *o_atom2, *n_atom2,
      *tmp_atom;
  atom_type h_atom;
  int i, j, k, ii, jj, kk;
  atom_list *llist, *alist;
      ...
      for (jj = j - 1; jj <= j + 1; jj++) {
        for (kk = k - 1; kk <= k + 1; kk++) {
          if (ii >= 0 && ii < xgrid && jj >= 0 && jj < ygrid && kk >= 0 &&
            ...
            llist = grid[ii][jj][kk];
            while (llist) {
              if (llist->atom->name[0] == 'O' && llist->atom->name[1] == ' ' &&
                  abs(llist->atom->res->locnum - n_atom1->res->locnum) > 2) {
                tmp_atom = llist->atom;
...
void rot_point_vector(real *x, real *y, real *z, real u, real v, real w,
                      real angle) {
...
int main(int argc, char **argv) {
  ...
  if (!name) {
    ...
    printf("  -g : use PDBSG as an input format (CA=C-alpha, SC or CM=side "
           "chain c.m.)\n\n");
    ...
    printf("  -i pdbfile : read the initial C-alpha coordinates from a PDB "
           "file\n");
    printf("  -t : save chain optimization trajectory to file "
           "<pdb_file.pdb.trajectory>\n");
    printf("  -u value : maximum shift from the restraint coordinates "
           "(default: 0.5A)\n\n");
    printf("  -e : rearrange backbone atoms (C, O are output after side chain) "
           "(default: off)\n");
    printf("  -f : preserve initial coordinates (default: off, implies '-c' on "
           "and '-n' off)\n");
    ...
    printf("  -o : don't attempt to fix excluded volume conflicts (default: "
           "on)\n");
  ...
  if (_VERBOSE)
    printf("%d residua read.\n", chain->nres);

  if (_PRESERVE)
    printf("Initial coordinates will be preserved.\n");
  ...
  if (_BB_REARRANGE) {
    if (_VERBOSE)
      printf("Rearranging backbone atoms (output: %s)...\n", out_name);
  }
  ...
  if (_VERBOSE)
    printf("Done. Reconstruction finished in %.3f s.\n",
           (real)0.001 * (1000. * (time1.time - time0.time) +
